...
// var selectCategory = dropdown.value
var multipleCategory = []
for (let i = 0; i < data.length; i++) {
multipleCategory.push(data[i].category)
  // dropdown.innerHTML += `<option value="${data[i].category}">${data[i].category}</option>`
    for (let j = 0; j < data[i].reviews.length; j++) {
      var card = document.createElement("div")
      card.className = "card";
      card.innerHTML = `<img class="card-img" src="./src/img/${data[i].title}.png" alt="">
  
    <div class="card-body">
        <h1 class="card-title">${data[i].title}</h1>
        <P class="card-sub-title">reviewerName: ${data[i].reviews[j].reviewerName} </P>
        <p class="card-info">Rating: ${data[i].reviews[j].rating}</p>
        <p class="card-info">comment: ${data[i].reviews[j].comment}</p>
         <p class="card-info">date: ${data[i].reviews[j].date}</p>
          <p class="card-info">reviewerEmail: ${data[i].reviews[j].reviewerEmail}</p>
...
var newCategory = []
for (let c = 0; c <multipleCategory.length; c++) {
  if (!newCategory.includes(multipleCategory[c])){
    newCategory.push(multipleCategory[c])
  }
  
}
console.log(newCategory)
for (let z = 0; z < newCategory.length; z++) {
 
  
  dropdown.innerHTML += `<option value="${newCategory[z]}">${newCategory[z]}</option>`;
}
var dropdownBrand = document.getElementById("dropdown-brand");

dropdown.addEventListener("click", function(){
  dropdownBrand.innerHTML = `<option value="">Select Brand</option>`;
  // var brand = dropdownBrand.value
  var selectCategory = dropdown.value;
  // dropdownBrand.innerHTML = `<option value="">Select Brand</option>`;
  for (let d = 0; d < data.length; d++) {
    if (selectCategory === data[d].category) {
      dropdownBrand.innerHTML += `<option value="${data[d].brand}">${data[d].brand}</option>`;
    }
  
}})

var container_brand = document.getElementById("container-brand");
dropdownBrand.addEventListener("click",function(){
  document.body.style.backgroundColor = "#363636";
  container_1.innerHTML = ""
  container_brand.innerHTML = ""
  var brand = dropdownBrand.value;
  for (let e = 0; e < data.length; e++) {
  if(brand == data[e].brand){
    var product_card = document.createElement("div")
    product_card.className = "product-card";
    product_card.innerHTML = `<div class="badge">Hot</div>
		<div class="product-tumb">
			<img src="./src/img/${data[e].title}.png" alt="">
		</div>
		<div class="product-details">
			<span class="product-catagory">${data[e].brand}</span>
			<h4><a href="">${data[e].title}</a></h4>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Vero, possimus nostrum!</p>
			<div class="product-bottom-details">
				<div class="product-price">price :${data[e].price}</div>
        <div class="product-price">discount :${data[e].discountPercentage}</div>
    ...
    container_brand.appendChild(product_card)
  }
  }
})
// var array 
